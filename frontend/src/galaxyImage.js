...
// ArrayBuffer of a ugriz cutout -> THREE.CanvasTexture (sRGB).
export function npyToTexture(buf) {
  ...
  const scales = bands.map((b) => bandPercentile(data, c, b, n, 0.995))
  ...
  for (let p = 0; p < n; p++) {
    const o = p * c
    img.data[p * 4 + 0] = 255 * stretch(data[o + bands[0]], scales[0])
    img.data[p * 4 + 1] = 255 * stretch(data[o + bands[1]], scales[1])
    img.data[p * 4 + 2] = 255 * stretch(data[o + bands[2]], scales[2])
    img.data[p * 4 + 3] = 255
  }
