...
You pushed branch **`task/cnn-02-build-cnn`**.

## PR (preferred)
GitHub: **base = `2026.6.17`**, compare = `task/cnn-02-build-cnn` → create + merge.

## CLI
```bash
git checkout 2026.6.17 && git pull origin 2026.6.17
git merge task/cnn-02-build-cnn
...
```bash
git checkout task/cnn-02-build-cnn
git merge origin/2026.6.17     # keep every function; delete the <<< === >>> markers
git add cnn/model.py && git commit && git push origin task/cnn-02-build-cnn
```
