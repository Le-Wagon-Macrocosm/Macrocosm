...
You pushed your work on branch **`task/08-predict-endpoint`** (the last cell of `task.ipynb` did this).
Now get it into the shared branch **`2026.6.17`**.

## Option A — Pull Request (preferred)
1. On GitHub you'll see a banner for `task/08-predict-endpoint` -> **Compare & pull request**.
2. Set **base = `2026.6.17`**, **compare = `task/08-predict-endpoint`**, create the PR.
...
git merge task/08-predict-endpoint
...
```bash
git checkout task/08-predict-endpoint
...
git add <file>
git commit
git push origin task/08-predict-endpoint            # your PR now merges cleanly
```
