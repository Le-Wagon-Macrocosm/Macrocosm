...
You pushed your work on branch **`task/04-model-load`** (the last cell of `task.ipynb` did this).
Now get it into the shared branch **`2026.6.17`**.

## Option A — Pull Request (preferred)
1. On GitHub you'll see a banner for `task/04-model-load` -> **Compare & pull request**.
2. Set **base = `2026.6.17`**, **compare = `task/04-model-load`**, create the PR.
...
```bash
git checkout task/04-model-load
...
git add <file>
git commit
git push origin task/04-model-load            # your PR now merges cleanly
```
