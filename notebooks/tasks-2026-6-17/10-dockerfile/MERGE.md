...
You pushed your work on branch **`task/10-dockerfile`** (the last cell of `task.ipynb` did this).
Now get it into the shared branch **`2026.6.17`**.

## Option A — Pull Request (preferred)
1. On GitHub you'll see a banner for `task/10-dockerfile` -> **Compare & pull request**.
2. Set **base = `2026.6.17`**, **compare = `task/10-dockerfile`**, create the PR.
...
```bash
git checkout task/10-dockerfile
...
git add <file>
git commit
git push origin task/10-dockerfile            # your PR now merges cleanly
```
