...
You pushed branch **`task/08-requirements`**. Get it into `2026.6.17`:

## Option A — Pull Request (preferred)
On GitHub: **base = `2026.6.17`**, **compare = `task/08-requirements`** → create + merge the PR.

## Option B — command line
```bash
git checkout 2026.6.17 && git pull origin 2026.6.17
git merge task/08-requirements
...
```bash
git checkout task/08-requirements
git merge origin/2026.6.17    # keep BOTH functions/routes; delete the <<< === >>> markers
git add <file> && git commit && git push origin task/08-requirements
```
