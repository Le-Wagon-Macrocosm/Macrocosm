...
You pushed your work on branch **`task/07-predict-fn`** (the last cell of `task.ipynb` did this).
Now get it into the shared branch **`2026.6.17`**.

## Option A — Pull Request (preferred)
1. On GitHub you'll see a banner for `task/07-predict-fn` -> **Compare & pull request**.
2. Set **base = `2026.6.17`**, **compare = `task/07-predict-fn`**, create the PR.
...
```bash
git checkout task/07-predict-fn
...
git add <file>
git commit
git push origin task/07-predict-fn            # your PR now merges cleanly
```
