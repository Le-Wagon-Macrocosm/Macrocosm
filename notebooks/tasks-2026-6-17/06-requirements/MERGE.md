...
You pushed your work on branch **`task/06-requirements`** (the last cell of `task.ipynb` did this).
Now get it into the shared branch **`2026.6.17`**.

## Option A — Pull Request (preferred)
1. On GitHub you'll see a banner for `task/06-requirements` -> **Compare & pull request**.
2. Set **base = `2026.6.17`**, **compare = `task/06-requirements`**, create the PR.
...
git merge task/06-requirements
...
```bash
git checkout task/06-requirements
...
git add <file>
git commit
git push origin task/06-requirements            # your PR now merges cleanly
```
