...
You pushed your work on branch **`task/03-preprocessing`** (the last cell of `task.ipynb` did this).
Now get it into the shared branch **`2026.6.17`**.

## Option A — Pull Request (preferred)
1. On GitHub you'll see a banner for `task/03-preprocessing` -> **Compare & pull request**.
2. Set **base = `2026.6.17`**, **compare = `task/03-preprocessing`**, create the PR.
...
git merge task/03-preprocessing
...
```bash
git checkout task/03-preprocessing
...
git add <file>
git commit
git push origin task/03-preprocessing            # your PR now merges cleanly
```
