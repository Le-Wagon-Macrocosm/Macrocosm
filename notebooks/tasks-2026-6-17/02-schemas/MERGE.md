...
You pushed your work on branch **`task/02-schemas`** (the last cell of `task.ipynb` did this).
Now get it into the shared branch **`2026.6.17`**.

## Option A — Pull Request (preferred)
1. On GitHub you'll see a banner for `task/02-schemas` -> **Compare & pull request**.
2. Set **base = `2026.6.17`**, **compare = `task/02-schemas`**, create the PR.
...
git add <file>
git commit
git push origin task/02-schemas            # your PR now merges cleanly
```
